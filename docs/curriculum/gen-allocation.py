...
# prescribed source per subject, grade-aware. Every entry traces to a named
# chapter of 00-LOCKED-DECISIONS.md. (ch2 = Programme One, ch3 = Two,
# ch4 = Three, ch4+ = the supplementary chapter the L-30 check forced.)
def rng(lo,hi,txt): return (lo,hi,txt)
SRC={
 'حفظ القرآن الكريم':[rng(1,12,'**المصحف — مطبعة الملك فهد، رواية حفص عن عاصم** · **كتاب القرآن وعلومه** (المدرسة)')],
 'التجويد':[rng(1,3,'بلا متن — تلقينًا'),rng(4,5,'**تحفة الأطفال والغلمان** — الجمزوري · فتح الأقفال (للمعلم)'),
            rng(6,9,'**المقدمة الجزرية** — ابن الجزري · الملخص المفيد — معبد · هداية القاري (للمعلم)'),
            rng(10,11,'**ورقة الإتقان والإقراء** (المدرسة) · هداية القاري'),rng(12,12,'صيانة — بلا متن')],
 ...
 'النشيد والمحفوظات':[rng(1,5,'مواد المدرسة'),rng(6,12,'**ديوان الشافعي · لامية ابن الوردي · تائية الإلبيري** · ملحة الإعراب ٧–٨ *(محفوظات لا متن تدريس)* · نظم الورقات · البيقونية · الرحبية')],
 'البحث والخطابة':[rng(4,9,'**دليل الإنشاء والخطابة** (المدرسة)'),
                   rng(10,12,'**دليل الإنشاء والخطابة** (المدرسة) · **جواهر الأدب** — أحمد الهاشمي')],
 ...
 'الترجمة':[rng(10,12,'مواد المدرسة — العربية ← اليوربا')],
 ...
 'الفقه':[rng(1,5,'مسار داخل كتاب الدراسات الإسلامية'),rng(6,7,'**المقدمة الأخضرية** — الأخضري *(الإطار مالكي مُعلَن)*'),
 ...
 'أصول الفقه':[rng(8,9,'تأصيل عملي داخل الفقه — بلا دلالات'),
               ...
               rng(12,12,'**مقاصد الشريعة** — ابن عاشور · الموافقات *(مقاطع)* · القواعد — الزرقا · **مجلة الأحكام ١–٩٩**')],
 'العقيدة':[rng(7,7,'**متن الأدلة** (المدرسة) *(بلا مذهب، عمدًا)* · **الأصول الثلاثة**'),
            rng(8,8,'**لمعة الاعتقاد** — ابن قدامة *(يُعلَن حنبليًّا)* · القواعد الأربع'),
            rng(9,9,'**العقيدة الطحاوية** — الإلهيات'),
            rng(10,10,'**أم البراهين** — السنوسي · **الواسطية** — ابن تيمية · نواقض الإسلام *(بعد الموانع)*'),
            rng(11,11,'**الاعتصام** — الشاطبي *(مقاطع)* · كتاب التوحيد *(وحدة محدودة)* · **كتاب الفِرَق** (المدرسة)'),
            rng(12,12,'**العقيدة الطحاوية** تمامًا')],
 'التوحيد':[rng(1,6,'مسار داخل كتاب الدراسات الإسلامية'),rng(7,12,'باب مسمّى داخل العقيدة')],
 'الحديث النبوي':[rng(1,6,'مسار داخل كتاب الدراسات الإسلامية'),
                  rng(7,7,'**الأربعون النووية** + **زيادات ابن رجب**'),
                  rng(8,9,'**عمدة الأحكام** — المقدسي · **الأدب المفرد** *(مختارات، ٩)*'),
                  rng(10,11,'**بلوغ المرام** — ابن حجر · **الموطأ** *(١٠)* · الشمائل *(١١)* · شرح معاني الآثار — الطحاوي'),
                  rng(12,12,'**البخاري ومسلم** *(كتب مختارة)* · **جامع العلوم والحكم** — ابن رجب')],
 'مصطلح الحديث':[rng(9,9,'**البيقونية** — محفوظة'),rng(10,12,'**نخبة الفكر ونزهة النظر** — ابن حجر · الرفع والتكميل — اللكنوي *(مرجع)*')],
...
def form_of(note,sub,g,host):
    n=note.strip()
    for f in ['مدمج','مضمّن','وحدة','مسار','دوراني','مستقل']:
...
def rows_for(g):
    ...
    R.append(dict(prog='القرآن',sub='حفظ القرآن الكريم',form='مستقل',slots='—',
        ...
        note=f'{290 if lower else 340} د/أسبوع · **خارج الخانات** · لا تُقتطع لأي مادة (L-20)',
...
for g in range(1,13):
    ...
    w(f'> **الساعة القرآنية:** {290 if lower else 340} د/أسبوع — **خارج هذه الخانات تمامًا**. '
