#!/usr/bin/env python3
"""Two-pass assembly for the AMIU Marketing & Communications Handbook
(MKT-001 through MKT-007). Reuses the exact reference.docx (fonts/styles)
...
Handbook, Student Policies Handbook, Operations Handbook, and Legal &
Compliance Handbook, imported directly rather than re-implemented, so all
eight flagship publications share one underlying pipeline and one design
system."""
...
from gen_mkt_handbook_body import POLICIES

FRONT = "assets/mkt_handbook_front.md"
BODY = "work_mkt_handbook_body.md"
BACK = "assets/mkt_handbook_back.md"
...
TOC_ENTRIES = [
    ...
    ("Marketing & Communications Policies Declaration", "Marketing & Communications Policies Declaration", 1),
]
for (num, code, title, steward, owner, authority, thesis, sections) in POLICIES:
    TOC_ENTRIES.append((f"{code} — {title}", f"Marketing &amp; Communications Handbook &#183; Policy {num} of 7", 1))
TOC_ENTRIES += [
    ("Publication Certification Statement", "Publication Certification Statement", 1),
]

BEFORE_TOC = {"Document Control", "Marketing & Communications Policies Declaration"}
...
def run_pandoc(md_path, docx_path):
    ...
    _retitle_header_footer(docx_path, "Marketing & Communications Handbook", "AMIU-MKTH-001")
...
if __name__ == "__main__":
    ...
    p1_md = assemble(build_toc_markdown(None), "assembled_mkth_pass1.md")
    run_pandoc(p1_md, "assembled_mkth_pass1.docx")
    run_soffice("assembled_mkth_pass1.docx", "/tmp/lo_mkth_pass1")
    page_map = extract_page_map("assembled_mkth_pass1.pdf", "assembled_mkth_pass1.txt")
    ...
    p2_md = assemble(build_toc_markdown(page_map), "AMIU_Marketing_Communications_Handbook_2028.md")
    run_pandoc(p2_md, "AMIU_Marketing_Communications_Handbook_2028.docx")
    run_soffice("AMIU_Marketing_Communications_Handbook_2028.docx", "/tmp/lo_mkth_pass2")
    print("DONE -> AMIU_Marketing_Communications_Handbook_2028.docx / .pdf")
