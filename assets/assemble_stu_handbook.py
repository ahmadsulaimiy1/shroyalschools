#!/usr/bin/env python3
"""Two-pass assembly for the AMIU Student Policies Handbook (STU-001 through
STU-023). Reuses the exact reference.docx (fonts/styles) and structural
post-processing passes already built and proven for the Blueprint,
Constitution, Institutional Governance Compendium, and Academic Handbook,
imported directly rather than re-implemented, so all five flagship
...
from gen_stu_handbook_body import POLICIES

FRONT = "assets/stu_handbook_front.md"
BODY = "work_stu_handbook_body.md"
BACK = "assets/stu_handbook_back.md"
...
TOC_ENTRIES = [
    ...
    ("Student Policies Declaration", "Student Policies Declaration", 1),
]
for (num, code, title, steward, owner, authority, thesis, sections) in POLICIES:
    TOC_ENTRIES.append((f"{code} — {title}", f"Student Policies Handbook &#183; Policy {num} of 23", 1))
TOC_ENTRIES += [
    ("Publication Certification Statement", "Publication Certification Statement", 1),
]

BEFORE_TOC = {"Document Control", "Student Policies Declaration"}
...
def run_pandoc(md_path, docx_path):
    ...
    _retitle_header_footer(docx_path, "Student Policies Handbook", "AMIU-STUH-001")
...
if __name__ == "__main__":
    ...
    p1_md = assemble(build_toc_markdown(None), "assembled_stuh_pass1.md")
    run_pandoc(p1_md, "assembled_stuh_pass1.docx")
    run_soffice("assembled_stuh_pass1.docx", "/tmp/lo_stuh_pass1")
    page_map = extract_page_map("assembled_stuh_pass1.pdf", "assembled_stuh_pass1.txt")
    ...
    p2_md = assemble(build_toc_markdown(page_map), "AMIU_Student_Policies_Handbook_2028.md")
    run_pandoc(p2_md, "AMIU_Student_Policies_Handbook_2028.docx")
    run_soffice("AMIU_Student_Policies_Handbook_2028.docx", "/tmp/lo_stuh_pass2")
    print("DONE -> AMIU_Student_Policies_Handbook_2028.docx / .pdf")
