#!/usr/bin/env python3
"""Two-pass assembly for the AMIU Waqf & Research Handbook (WAQ-001
through WAQ-009). Reuses the exact reference.docx (fonts/styles) and
...
Compliance Handbook, and Marketing & Communications Handbook, imported
directly rather than re-implemented, so all nine flagship publications
...
from gen_waq_handbook_body import POLICIES

FRONT = "assets/waq_handbook_front.md"
BODY = "work_waq_handbook_body.md"
BACK = "assets/waq_handbook_back.md"
...
TOC_ENTRIES = [
    ...
    ("Waqf & Research Policies Declaration", "Waqf & Research Policies Declaration", 1),
]
for (num, code, title, steward, owner, authority, thesis, sections) in POLICIES:
    TOC_ENTRIES.append((f"{code} — {title}", f"Waqf &amp; Research Handbook &#183; Policy {num} of 9", 1))
TOC_ENTRIES += [
    ("Publication Certification Statement", "Publication Certification Statement", 1),
]

BEFORE_TOC = {"Document Control", "Waqf & Research Policies Declaration"}
...
def run_pandoc(md_path, docx_path):
    ...
    _retitle_header_footer(docx_path, "Waqf & Research Handbook", "AMIU-WAQH-001")
...
if __name__ == "__main__":
    ...
    p1_md = assemble(build_toc_markdown(None), "assembled_waqh_pass1.md")
    run_pandoc(p1_md, "assembled_waqh_pass1.docx")
    run_soffice("assembled_waqh_pass1.docx", "/tmp/lo_waqh_pass1")
    page_map = extract_page_map("assembled_waqh_pass1.pdf", "assembled_waqh_pass1.txt")
    ...
    p2_md = assemble(build_toc_markdown(page_map), "AMIU_Waqf_Research_Handbook_2028.md")
    run_pandoc(p2_md, "AMIU_Waqf_Research_Handbook_2028.docx")
    run_soffice("AMIU_Waqf_Research_Handbook_2028.docx", "/tmp/lo_waqh_pass2")
    print("DONE -> AMIU_Waqf_Research_Handbook_2028.docx / .pdf")
