#!/usr/bin/env python3
"""Two-pass assembly for the AMIU Academic Handbook (ACA-001 through ACA-019).
Reuses the exact reference.docx (fonts/styles) and structural post-processing
passes already built and proven for the Blueprint, Constitution, and
Institutional Governance Compendium, imported directly rather than
re-implemented, so all four flagship publications share one underlying
pipeline and one design system."""
...
from gen_aca_handbook_body import POLICIES

FRONT = "assets/aca_handbook_front.md"
BODY = "work_aca_handbook_body.md"
BACK = "assets/aca_handbook_back.md"
...
TOC_ENTRIES = [
    ...
    ("Academic Declaration", "Academic Declaration", 1),
]
for (num, code, title, steward, owner, authority, thesis, sections) in POLICIES:
    TOC_ENTRIES.append((f"{code} — {title}", f"ACADEMIC HANDBOOK &#183; POLICY {num} OF 19", 1))
TOC_ENTRIES += [
    ("Publication Certification Statement", "Publication Certification Statement", 1),
]

BEFORE_TOC = {"Document Control", "Academic Declaration"}
...
def run_pandoc(md_path, docx_path):
    ...
    _retitle_header_footer(docx_path, "Academic Handbook", "AMIU-ACAH-001")
...
if __name__ == "__main__":
    ...
    p1_md = assemble(build_toc_markdown(None), "assembled_acah_pass1.md")
    run_pandoc(p1_md, "assembled_acah_pass1.docx")
    run_soffice("assembled_acah_pass1.docx", "/tmp/lo_acah_pass1")
    page_map = extract_page_map("assembled_acah_pass1.pdf", "assembled_acah_pass1.txt")
    ...
    p2_md = assemble(build_toc_markdown(page_map), "AMIU_Academic_Handbook_2028.md")
    run_pandoc(p2_md, "AMIU_Academic_Handbook_2028.docx")
    run_soffice("AMIU_Academic_Handbook_2028.docx", "/tmp/lo_acah_pass2")
    print("DONE -> AMIU_Academic_Handbook_2028.docx / .pdf")
