#!/usr/bin/env python3
"""Two-pass assembly for the AMIU Legal & Compliance Handbook (LEG-001
through LEG-011). Reuses the exact reference.docx (fonts/styles) and
structural post-processing passes already built and proven for the
Blueprint, Constitution, Institutional Governance Compendium, Academic
Handbook, Student Policies Handbook, and Operations Handbook, imported
directly rather than re-implemented, so all seven flagship publications
share one underlying pipeline and one design system."""
...
from gen_leg_handbook_body import POLICIES

FRONT = "assets/leg_handbook_front.md"
BODY = "work_leg_handbook_body.md"
BACK = "assets/leg_handbook_back.md"
...
TOC_ENTRIES = [
    ...
    ("Legal & Compliance Policies Declaration", "Legal & Compliance Policies Declaration", 1),
]
for (num, code, title, steward, owner, authority, thesis, sections) in POLICIES:
    TOC_ENTRIES.append((f"{code} — {title}", f"Legal &amp; Compliance Handbook &#183; Policy {num} of 11", 1))
TOC_ENTRIES += [
    ("Publication Certification Statement", "Publication Certification Statement", 1),
]

BEFORE_TOC = {"Document Control", "Legal & Compliance Policies Declaration"}
...
def run_pandoc(md_path, docx_path):
    ...
    _retitle_header_footer(docx_path, "Legal & Compliance Handbook", "AMIU-LEGH-001")
...
if __name__ == "__main__":
    ...
    p1_md = assemble(build_toc_markdown(None), "assembled_legh_pass1.md")
    run_pandoc(p1_md, "assembled_legh_pass1.docx")
    run_soffice("assembled_legh_pass1.docx", "/tmp/lo_legh_pass1")
    page_map = extract_page_map("assembled_legh_pass1.pdf", "assembled_legh_pass1.txt")
    ...
    p2_md = assemble(build_toc_markdown(page_map), "AMIU_Legal_Compliance_Handbook_2028.md")
    run_pandoc(p2_md, "AMIU_Legal_Compliance_Handbook_2028.docx")
    run_soffice("AMIU_Legal_Compliance_Handbook_2028.docx", "/tmp/lo_legh_pass2")
    print("DONE -> AMIU_Legal_Compliance_Handbook_2028.docx / .pdf")
